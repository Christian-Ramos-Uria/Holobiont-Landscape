...
k <- round(nrow(human.samples) / 10)
knn <- dist2knn(js.dist, k = k)
human.samples[, knn := knn[sample]]
ggplot(human.samples, aes(x = progression, y = knn)) +
  geom_smooth() +
  geom_point(aes(color = delivery)) +
  facet_wrap(~ subject)

#' Diarrhea samples only showing disease duration:
setkey(human.samples, delivery)
theme_set(theme_cowplot())
ggplot(human.samples["vaginal"], aes(x = month, y = knn)) +
  geom_smooth() +
  geom_point() +
  facet_wrap(~ subject)
...
cholera.vertices <- function(map) {
  map[, .(mean.knn = mean(knn),
          f.state = sum(delivery == "vaginal") / .N,
          mean.t = mean(month),
          size = .N),
...
mpr <- cholera.mapper(js.dist, human.samples, ftr)
...
# validate
subsets <- validate(js.dist, human.samples, cholera.mapper, rs, nrep)
...
write.validation.plot(paste0(figs.dir, 'sup_fighuman2.pdf'), last_plot())
