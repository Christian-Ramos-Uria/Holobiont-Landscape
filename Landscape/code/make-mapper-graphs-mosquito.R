...
mosquito[, freq := count / sum(count), by = sample]
#mosquito.samples <- unique(mosquito[, .(sample, replicate, diagnosis, id, generation)])
mosquito.samples[, idx := frank(generation), by = replicate]
mosquito.samples[, progression := idx / max(idx), by = replicate]
distribs <- dcast(mosquito, sample ~ fun, value.var = "freq", fill = 0)
sample.names <- distribs$sample
distribs <- as.matrix(distribs[, -1])

# import js distance
jsd.file <- paste0(jsd.dir, "mosquito.txt")
if (!file.exists(jsd.file)) {
  print("Making mosquito distance matrix...")
  source("make-jsds-mosquito.R")
}
jsd <- fread(jsd.file)
...
k <- round(nrow(mosquito.samples) / 10)
knn <- dist2knn(js.dist, k = k)
mosquito.samples[, knn := knn[sample]]
ggplot(mosquito.samples, aes(x = progression, y = knn)) +
  geom_smooth() +
  geom_point(aes(color = treatment)) +
  facet_wrap(~ replicate)

#' Diarrhea samples only showing disease duration:
setkey(mosquito.samples, treatment)
theme_set(theme_cowplot())
ggplot(mosquito.samples["Field"], aes(x = generation, y = knn)) +
  geom_smooth() +
  geom_point() +
  facet_wrap(~ replicate)
...
mpr <- cholera.mapper(js.dist, mosquito.samples, ftr)
...
## validate
#subsets <- validate(js.dist, mosquito.samples, cholera.mapper, rs, nrep)
...
#write.validation.plot(paste0(figs.dir, 'sup_figmosquito.pdf'), last_plot())
