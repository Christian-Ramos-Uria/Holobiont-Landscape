...
# Take a random subset (optional)
p2basin <- p2basin[delivery == "vaginal"]
p2basin <- p2basin[sample(1:length(rownames(p2basin)), 21, replace = FALSE)]
...
plot_grid(plot_grid(fstate, basin, labels = "AUTO", align = "h", nrow = 1),
          ...
          #label_y = c(1, 1.5),
          ncol = 1)
save_plot(last_plot(), filename = paste0(figs.dir, "/fighF2sub.png"),
