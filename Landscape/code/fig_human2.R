...
human.mapper <- read.mapper.graph(paste0(output.dir, "human2"))
...
d2basin <- p2basin[, .(N = sum(N)), by = .(subject, delivery, basin)]
d2basin[, frac := N / sum(N), by = .(subject, delivery)]
setkey(d2basin, delivery)
d2basin["vaginal"] %>%
...
human.persistence <- split(human.v2p, by = c("subject", "delivery")) %>%
  lapply(function(df) basin.persistence(df$basin, df$month, scale = TRUE)) %>%
  rbindlist(idcol = "group") %>%
  .[, c("subject", "delivery") := tstrsplit(group, "\\.")] %>%
  .[, basin := as.factor(as.numeric(basin))]
setkey(human.persistence, delivery)
human.persistence["vaginal"] %>%
  .[!is.na(basin)] %>%
...
plot_grid(plot_grid(fstate, basin,
          ...
          label_y = c(1, 1.2, 1),
          ncol = 1)
save_plot(last_plot(), filename = paste0(figs.dir, "/figh2.pdf"),
