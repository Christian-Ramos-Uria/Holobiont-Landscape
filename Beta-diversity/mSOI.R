...
#' @param m.vector species-specific immigration probabilities

#' @param i.vector initial species abundances, a vector with length < I 
...
msoi<-function(N, I, A, m.vector=runif(N), i.vector=NULL, e.vector=runif(N), tend, K=5, perturb=NULL){
  
  
  
  results<-generate.parameters(N, I, A, m.vector, i.vector, e.vector)
...
generate.parameters<-function(N, I, A, m.vector, i.vector, e.vector){
